                    ...
                    <ul class="space-y-2 max-h-60 overflow-y-auto p-2 bg-white rounded-lg shadow-sm">
                        @foreach($invited as $u)
                            <li class="flex items-center gap-3">
                                ...
                                <span class="font-medium text-sm">{{ $u->ID_USER }}</span>
                                <span class="font-medium text-sm">{{ $nhom->ID_NHOM }}</span>
                                ...
                                <form method="POST" action="{{ url('/invite') }}" class="ml-auto">
                                    ...
                                    <button type="submit"
                                        onclick="setTimeout(() => { this.disabled = true; this.classList.add('opacity-50'); this.innerText = 'Đã mời'; }, 10);">
                                        Mời
                                    </button>

    ...
    <div class="relative w-full">
        ...
        <form id="anhBiaForm" action="{{ route('profile.upload_anhbia') }}" method="POST" enctype="multipart/form-data">
            ...
            <label for="anhBiaInput" style="cursor: pointer;">
                <div class="w-full max-h-60 bg-gray-100 flex justify-center items-center overflow-hidden rounded-lg">
                    <img
        ...
        {{-- Avatar --}}
        <div class="absolute -translate-x-1/2 left-1/2 -bottom-16">
            <form id="avatarForm" action="{{ route('profile.upload_avatar') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <label for="avatarInput" class="block w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-md cursor-pointer">
                    <img

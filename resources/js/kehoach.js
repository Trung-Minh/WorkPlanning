...
function loadTemplate(name, el = null) {
    ...
    document.getElementById("pageTitle").textContent =
        localStorage.getItem(name + "_title") ||
        `Mẫu ${name.replace("mau", "")}`;
    sectionCount = 0;
...
function createNewTemplate() {
    ...
    .then(data => {
        if (data.success) {
            ...
            a.href = "#";
            a.className = "block px-3 py-2 rounded editable-tab";
            a.textContent = data.ten;
            a.dataset.id = key;
            a.onclick = function () {
...
function createColumn(id, label) {
    ...
    title.className =
        "font-bold mb-3 flex justify-between items-center text-lg";
...
function addForm(containerId) {
    ...
    title.textContent =
        localStorage.getItem(`${formId}_title`) || `Mục ${index}`;
...
window.deleteCurrentForm = deleteCurrentForm;
window.clearDeadline = clearDeadline;
...
function addNewColumn() {
    const id = `section_${sectionCount++}`;
    console.log("👉 Đã gọi addNewColumn, id:", id);
    counters[id] = 0;
    createColumn(id, `Tiêu đề ${sectionCount}`);
}
